...
def delete(request):
    ...
    for instance in instances:
        # only myworkflow admin can delete
...
def start_wf(request):
    template_name = 'myworkflow/start_wf.html'
    ...
    menudata = menu_data(gol.get_value('menudata'))
    menu_list = menudata
    ctx = {
...
def report_list(request):
    template_name = 'myworkflow/report_list.html'
    categories = ProcessCategory.objects.filter(
        is_active=True).order_by('oid')
